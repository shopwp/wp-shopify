<?php
  ...
  usort($data->product['images'], array($Utils, "sort_product_images"));


  $i = 0;
  $len = count($data->product['images']);
  $amountOfThumbs = $len;
  ...
  */
  if ($amountOfThumbs < 1) {
    ...
    foreach ($data->product['images'] as $key => $image) {
      ...
      $image = Images::get_image_details_from_image($image, $data->product);
      ...
      echo apply_filters('wps_product_img', $productImg, $data->product, $i);
