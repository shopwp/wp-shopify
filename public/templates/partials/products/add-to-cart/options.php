...
if (count($data->product['options']) === 1) {
  ...
  $col = count($data->product['options']);
}

$uid = 0;
$optionNumber = 1;
$options = Utils::wps_sort_by($data->product['options'], 'position');
...
<?php foreach ($options as $key => $option) { ?>
  ...
  <div
    class="wps-btn-dropdown wps-col wps-col-<?php echo $col; ?> <?php echo apply_filters('wps_options_class', ''); ?>"
    data-selected="false"
    data-open="false"
    data-selected-val=""
