  ...
  <div class="wps-admin-section">
    ...
    <form method="post" name="cleanup_options" action="options.php" id="wps-connect" class="wps-admin-form">
      ...
      <div class="wps-form-group">
        ...
        <input required <?php echo $connected ? 'disabled' : ''; ?> type="text" class="regular-text <?php echo $connected ? 'valid' : ''; ?>" id="<?php echo $this->config->settings_connection_option_name; ?>_shared_secret" name="shared_secret" value="<?php if(!empty($connection->shared_secret)) echo $connection->shared_secret; ?>" placeholder=""> <span class="wps-help-tip wps-help-tip-inline-no-position" title="<?php esc_attr_e( 'To generate a Shared Secret you must create a "Private App" within your Shopify account. The Shared Secret is used to validate webhook requests and provide security to WP Shopify.', 'wp-shopify' ); ?>"></span> <div class="wps-form-icon wps-animated"></div>
      ...
      <div class="wps-form-group">
        ...
        <input required <?php echo $connected ? 'disabled' : ''; ?> type="text" class="regular-text <?php echo $connected ? 'valid' : ''; ?>" id="<?php echo $this->config->settings_connection_option_name; ?>_js_access_token" name="js_access_token" value="<?php if(!empty($connection->js_access_token)) echo $connection->js_access_token; ?>" placeholder=""> <span class="wps-help-tip wps-help-tip-inline-no-position" title="<?php esc_attr_e( 'The Storefront Access Token is used to create the front-end cart experience.', 'wp-shopify' ); ?>"></span><div class="wps-form-icon wps-animated"></div>
