...
class Settings_Products extends \WPS\API {
  ...
  /*
	...
	*/
	public function update_setting_products_heading_toggle($request) {
		...
		if ( is_string($enabled) ) {
			return $this->error( $request->get_route(), 'Failed to update products heading toggle due to invalid type', 500);
		}

		$update_result = $this->DB_Settings_General->update_products_heading_toggle($enabled);

		if (!$update_result) {
			return $this->error( $request->get_route(), 'Failed to update products heading toggle', 500);
		}
	...
	*/
	public function update_setting_products_heading($request) {
		...
		if ( !is_string($heading) ) {
			return $this->error( $request->get_route(), 'Failed to update products heading due to invalid type', 500);
		}

		$update_result = $this->DB_Settings_General->update_products_heading($heading);

		if (!$update_result) {
			return $this->error( $request->get_route(), 'Failed to update products heading', 500);
		}
	...
	*/
	public function update_setting_products_images_sizing_toggle($request) {
		...
		if ( is_string($toggle) ) {
			return $this->error( $request->get_route(), 'Failed to update products images sizing toggle due to invalid type', 500);
		}

		$update_result = $this->DB_Settings_General->update_products_images_sizing_toggle($toggle);

		if (!$update_result) {
			return $this->error( $request->get_route(), 'Failed to update products images sizing toggle', 500);
		}
	...
	*/
	public function update_setting_products_images_sizing_width($request) {
		...
		if ( is_string($width) ) {
			return $this->error( $request->get_route(), 'Failed to update products images sizing width due to invalid type', 500);
		}

		$update_result = $this->DB_Settings_General->update_products_images_sizing_width($width);

		if (!$update_result) {
			return $this->error( $request->get_route(), 'Failed to update products images sizing width', 500);
		}
	...
	*/
	public function update_setting_products_images_sizing_height($request) {
		...
		if ( is_string($height) ) {
			return $this->error( $request->get_route(), 'Failed to update products images sizing height due to invalid type', 500);
		}

		$update_result = $this->DB_Settings_General->update_products_images_sizing_height($height);

		if (!$update_result) {
			return $this->error( $request->get_route(), 'Failed to update products images sizing height', 500);
		}
	...
	*/
	public function update_setting_products_images_sizing_crop($request) {
		...
		if ( !is_string($crop) ) {
			return $this->error( $request->get_route(), 'Failed to update products images sizing crop due to invalid type', 500);
		}

		$update_result = $this->DB_Settings_General->update_products_images_sizing_crop($crop);

		if (!$update_result) {
			return $this->error( $request->get_route(), 'Failed to update products images sizing crop', 500);
		}
	...
	*/
	public function update_setting_products_images_sizing_scale($request) {
		...
		if ( !is_int($scale) ) {
			return $this->error( $request->get_route(), 'Failed to update products images scale value due to invalid type', 500);
		}

		$update_result = $this->DB_Settings_General->update_products_images_sizing_scale($scale);

		if (!$update_result) {
			return $this->error( $request->get_route(), 'Failed to update products images scale value', 500);
		}
