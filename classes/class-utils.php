	    ...
	    */
	    ...
	    if (get_transient('wps_product_price_id_' . $productID)) {
	      ...
	      if (empty($currencySymbol[0])) {
	        $symbol = '$'; // default fallback incase user doesn't sync shop data
	      ...
	      $priceey = '<span class="wps-product-price-currency" itemprop="priceCurrency">' . $symbol . '</span>' . '<span itemprop="price" class="wps-product-individual-price">' . $price .'</span>' . '<span itemprop="priceCurrency" class="wps-product-individual-price-format">' . trim($currencySymbol[1]) .'</span>';
